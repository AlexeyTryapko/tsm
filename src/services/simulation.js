...
export const start = chart => {
	...
	if(step<chart.globalProperties['executionTime']/chart.globalProperties['quantizationPeriod']){
	    let func_names = {
	    ...
	    function calc(idx) {
	        ...
	        for (let [row_idx, row] of matrix.entries()) {
	            if (row[idx] !== 0) {
	                ...
	                x.push({name:get_node(row_idx).properties.name,data:Y[row_idx]});
	            }
	        }
	        
	        let node = get_node(idx)

	        return func_names[node.type](x, chart.globalProperties,node.properties, step); //TODO use step
	    }
